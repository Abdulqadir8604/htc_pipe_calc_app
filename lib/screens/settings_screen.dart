...
class _SettingsScreenState extends State<SettingsScreen> {
  ...
  void initState() {
    ...
    _config = CalculatorConfig(
      sizes: widget.config.sizes.map((size) => SizeData(
        size: size.size,
        price: size.price,
        fittings: size.fittings.map((fitting) => FittingPrice(
    ...
    _profitMarginController.text = (_config.profitMargin * 100).toStringAsFixed(0);
  }
  ...
  void _updateProfitMargin() {
    ...
    if (value != null && value >= 0 && value <= 100) {
      ...
      CalculatorService.updateProfitMargin(value / 100);
    }
  }

  void _updateSizePrice(int sizeIndex, double newPrice) {
    setState(() {
      _config.sizes[sizeIndex].price = newPrice;
    });
    CalculatorService.updateSizePrice(_config.sizes[sizeIndex].size, newPrice);
  }

  void _updateFittingPrice(int sizeIndex, int fittingIndex, double newPrice) {
    setState(() {
      _config.sizes[sizeIndex].fittings[fittingIndex].price = newPrice;
    });
    CalculatorService.updateFittingPrice(
                    ...
                    child: Column(
                      ...
                      children: [
                              ...
                              Container(
                                ...
                                child: TextFormField(
                                  ...
                                  decoration: const InputDecoration(
                                    ...
                                    suffixText: '%',
                                    hintText: 'Enter profit margin percentage',
                                    hintStyle: TextStyle(color: Colors.grey),
                                    prefixIcon: Icon(Icons.percent, color: Color(0xFFF39C12)),
                                  ),
                        ...
                        Container(
                          ...
                          child: const Row(
                            children: [
                              Icon(Icons.build, color: Color(0xFFF1C40F), size: 24),
                              SizedBox(width: 12),
                              Text(
                        ...
                        Expanded(
                          child: ListView.builder(
                            itemCount: _config.sizes.length,
                            itemBuilder: (context, sizeIndex) {
                              final size = _config.sizes[sizeIndex];
                              return Container(
                                        ...
                                        child: Column(
                                          children: [
                                            ...
                                            Container(
                                              ...
                                              child: Row(
                                                children: [
                                                  const Icon(Icons.straighten, color: Color(0xFF3498DB)),
                                                  const SizedBox(width: 12),
                                                  Expanded(
                                                    child: Text(
                                                  ...
                                                  SizedBox(
                                                    width: 120,
                                                    child: TextFormField(
                                                      initialValue: size.price.toStringAsFixed(0),
                                                      keyboardType: TextInputType.number,
                                                      inputFormatters: [
                                                        FilteringTextInputFormatter.allow(RegExp(r'^\d+\.?\d{0,2}')),
                                                      ],
                                                      ...
                                                      onChanged: (value) {
                                                        ...
                                                        if (price != null && price >= 0) {
                                                          _updateSizePrice(sizeIndex, price);
                                                        }
                                                      },
                                                    ),
                                                  ),
                                            ...
                                            // Fittings
                                            ...size.fittings.asMap().entries.map((entry) {
                                              final fittingIndex = entry.key;
                                              final fitting = entry.value;
                                              return Container(
                                                padding: const EdgeInsets.all(16),
                                                decoration: BoxDecoration(
                                                  color: fittingIndex.isEven ? Colors.white : const Color(0xFFF8F9FA),
                                                  border: const Border(
                                                ...
                                                child: Row(
                                                  children: [
                                                    ...
                                                    SizedBox(
                                                      width: 120,
                                                      child: TextFormField(
                                                        initialValue: fitting.price.toStringAsFixed(0),
                                                        keyboardType: TextInputType.number,
                                                        inputFormatters: [
                                                          FilteringTextInputFormatter.allow(RegExp(r'^\d+\.?\d{0,2}')),
                                                        ],
                                                        ...
                                                        onChanged: (value) {
                                                          final price = double.tryParse(value);
                                                          if (price != null && price >= 0) {
                                                            _updateFittingPrice(sizeIndex, fittingIndex, price);
                                                          }
                                                        },
                                                      ),
                                                    ),
